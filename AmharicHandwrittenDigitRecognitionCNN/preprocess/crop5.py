...
def load_image(_dir, num):
    img = Image.open(_dir)
    if num in [495, 497, 498, 509]:
        img = img.rotate(-1.5)
    else:
        img = img.rotate(-2.5)
    return img
...
def crop(img, pleft=3.8, ptop=26.5, pright=7, pbottom=6, number=None):
    if number in [495, 509]:
        pleft, ptop, pright, pbottom = 5, 27.5, 6, 5
    elif number in [497, 498]:
        pleft, ptop, pright, pbottom = 5, 27.8, 6, 4.5
...
for COUNTER, img_dir in image_list.items():
	img = crop(load_image(img_dir, COUNTER), number=COUNTER)
